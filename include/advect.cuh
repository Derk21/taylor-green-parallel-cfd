...
__host__ __device__ void interpolateVelocity(double x_d, double y_d,  const double *periodic_grid, const double *velocity_grid, double * velocity_grid_next, const int n = NUM_N, int m=M, const double dx = DX);
...
void advectSemiLagrange(
    ...
    const double dt, int n=NUM_N, int m=M,double dx=DX);
...
void advectMacCormack(
    ...
    const double *periodic_grid, 
    const double dt, int n=NUM_N, int m=M,const double dx=DX);
